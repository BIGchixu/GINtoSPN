...
#' @param text.show Character. The text to show on the node when imported into an editor.
...
#' @export
adjustNode_color_size<-function(g,input.gs,node.color=c("yellow","brown","grey","blue","orange","red"), node.size=c(5,3,7),cdglist=cdglist){
  ...
  tmp.color[node.tmp %in% cdglist] = node.color[6]
...
add_prcFormation_and_GRN<-function(g.subgraph,input.gs,graph.grn,cdglist){
  ...
  myComplex<-V(g.subgraph)$name[grep("_",V(g.subgraph)$name)]
